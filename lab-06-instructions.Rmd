...
```{r pivot_example}
df = tibble(time = seq(100), foo = -1 + 0.1 * time - 0.001 * time^2, bar = sin(time / 10))
...
`degas_spinup`). Leave `degas_sim` at 7.5 and start out by setting 
`degas_sim = 10`.
...
a) Does an increase in CO~2~ degassing drive atmospheric CO~2~ up or down?
   ...
   the change in `co2_atmos` for a time-step in the model is less than 
   1 ppm.
...
b) Check that the model balances silicate weathering against CO~2~ degassing
   ...
   **Hint:** This is a good place to use the `pivot_longer` function as I 
   described above.
